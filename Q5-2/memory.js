...
function deal_cards(card_number) {
    ...
    for (let card_index = 1; card_index < card_number; card_index++) {
...
function click_card(card) {
  var src = card.getAttribute("src");
  var row = card.getAttribute("row");
  var col = card.getAttribute("col");

  var image = document.getElementById("img-" + row + "." + col);

                let card_list = [];
                for (let card_index = 1; card_index < 4; card_index++) {
                  card_list.push(`img/image${card_index}.png`);
                  card_list.push(`img/image${card_index}.png`);
                }
                shuffle(card_list);
  ...
  image.setAttribute("src", board[row][col]);
}
